...
for i in range(0,len(total_fold)):
    temp_result=[[],[]]
    print('Start Subject {}'.format(i))
    data_fold=[]
    data_fold.append(total_fold[i])
    data_x, data_y  = read_mat(data_fold)
    for j in range(0,4):
        ...
        history = model.fit(train_x, train_y,epochs=8,validation_data=(test_x,test_y),verbose=0)
        temp_result[0].append(history.history['acc'][7])
        temp_result[1].append(history.history['val_acc'][7])
    print('Train Accuracy:{}'.format(temp_result[0]))
    print('Test Accuracy:{}'.format(temp_result[1]))
    result[0].append(sum(temp_result[0])/len(temp_result[0]))
    result[1].append(sum(temp_result[1])/len(temp_result[1]))
print('Average Train Accuracy for each subject:{}'.format(result[0]))
print('Average Test Accuracy for each subject:{}'.format(result[1]))
avtrain=sum(result[0])/len(result[0])
avtest = sum(result[1]) / len(result[1])
print('Overall Average Train Accuracy:{}'.format(avtrain))
print('Overall Average Test Accuracy:{}'.format(avtest))
